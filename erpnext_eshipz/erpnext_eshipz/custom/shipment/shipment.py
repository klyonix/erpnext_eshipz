...
@frappe.whitelist()
def make_shipment_from_sales_invoice(source_name, target_doc=None):
...
def create_rule_based_shipment(docname, item_data=None):
    ...
    for dn in doc.get("shipment_delivery_note"):    
        delivery_note = frappe.get_doc('Delivery Note', dn.kly_shipment_reference) 
...
@frappe.whitelist()
def get_delivery_note_items(delivery_note, dtype):
    if not frappe.has_permission(dtype, 'read', delivery_note):
        raise frappe.PermissionError

    # Fetch the document using the provided name and dtype
    doc = frappe.get_doc(dtype, delivery_note)

    # Get meta of the parent doctype
    meta = frappe.get_meta(doc.doctype)

    # Get child table field (assuming first one or modify logic to get specific)
    child_table_fields = [df for df in meta.fields if df.fieldtype == 'Table']
    if not child_table_fields:
        frappe.throw("No child tables found in the document.")

    child_table = child_table_fields[0]
    child_doctype = child_table.options

    # Check which fields exist in the child doctype
    child_meta = frappe.get_meta(child_doctype)
    available_fields = {df.fieldname for df in child_meta.fields}

    # Define fields you want, check if they exist
    fields_to_fetch = ['item_name', 'qty', 'uom', 'amount']
    if 'gst_hsn_code' in available_fields:
        fields_to_fetch.append('gst_hsn_code')

    # Fetch child records
    items = frappe.get_all(child_doctype,
        filters={'parent': delivery_note},
        fields=fields_to_fetch
    )

    # Ensure gst_hsn_code is present (even as empty string)
    for item in items:
        if 'gst_hsn_code' not in item:
            item['gst_hsn_code'] = ""
...
def create_shipment(docname, selected_service, item_data=None):
    ...
    for dn in doc.get("shipment_delivery_note"):
        if doc.doctype == "Delivery Note":
            ...
            document = frappe.get_doc(dn.kly_shipment_doctype, dn.kly_shipment_reference)
            if hasattr(document, 'items'):
                for item in document.items:
                    invoice_number = document.name
                    invoice_numbers.add(invoice_number)
                    invoice_date = getattr(document, 'posting_date', '')
                    invoice_currency = getattr(document, 'currency', 'INR')
                    invoice_value = getattr(document, 'grand_total',
                                        getattr(document, 'base_grand_total', 0))
                    ewaybill_number = getattr(document, 'ewaybill', '')
                    if ewaybill_number:
                        ewaybill_date = frappe.get_value('e-Waybill Log', ewaybill_number, 'created_on')
                    else:
                        ewaybill_date = ""

                    hsn_code = getattr(item, 'gst_hsn_code', '')
                    item_key = (item.item_name, item.uom, hsn_code, item.qty, item.amount)
                    consolidated_items[item_key]["weight"] += item.qty if item.uom == "Kg" else 1
                    consolidated_items[item_key]["amount"] += item.amount
