...
frappe.ui.form.on('Shipment', {
	refresh: function(frm) {
		...
		console.log("Shipment form refreshed", frm.doc);
	    frappe.call({
		...
		callback: function(r) {
			console.log("eShipz Settings enabled value:", r.message);
			...
			if (frm.doc.docstatus == 1 && !frm.doc.awb_number) {
				...
				callback: function(r) {
					console.log("enable_allocation value:", r.message);
					...
					frm.add_custom_button(__('Create Rule Based Shipment'), function() {
					    let parcel_count = frm.doc.shipment_parcel.length;
						 console.log("Parcel count for rule-based shipment:", parcel_count);
					...
					frm.add_custom_button(__('Create Shipment'), function() {
						...
						callback: function(r) {
							 console.log("Available services fetched:", r.message);
						    if (r.message) {
			...
			if (frm.doc.docstatus == 1 && frm.doc.awb_number && frm.doc.status != 'Cancelled') {
			    frm.add_custom_button(__('Download/Print Label'), function() {
					console.log("Creating shipment action buttons for AWB present");
...
function generate_parcel_selection_html(parcel_count, delivery_note_items) {
    ...
    for (let i = 1; i <= parcel_count; i++) {
        ...
        if (delivery_note_items && delivery_note_items.length) {
            html += `<div>
                <input type="checkbox" class="select-all" data-parcel="${i}" onchange="window.toggle_all_items(this)">
    ...
    window.toggle_all_items = function(selectAllCheckbox) {
    let parcel = selectAllCheckbox.getAttribute('data-parcel');
    let isChecked = selectAllCheckbox.checked;

    $(`.parcel-item[data-parcel="${parcel}"]`).each(function () {
        this.checked = isChecked;
        this.disabled = false;
    });

    // Trigger duplicate check logic when selecting all
    if (isChecked) {
        $(`.parcel-item[data-parcel="${parcel}"]`).each(function () {
            window.prevent_duplicate_selection(this);
        });
    }
};
